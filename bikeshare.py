...
def main():
    ...
    st.write("### Table structure for {} bike trips (first five rows)".format(
        city.title()), df.head())

    missing_values = df.isnull().sum().where(lambda x: x > 0).dropna()

    if not missing_values.empty:
        st.write("### Missing values per column", missing_values.sort_values(
            ascending=False).rename("Number of missing values"))

    # Visualize data about trips before month/day filters are applied
    trip_duration_per_month = df[[
        'Trip Duration', 'Month']].groupby('Month').sum()

    trip_duration_per_month_df = trip_duration_per_month.reset_index()

    trip_duration_per_month_df['Trip Duration'] = pd.Series(
        map(lambda x: round(x / (3600 * 24), 2), trip_duration_per_month_df['Trip Duration']))

    st.markdown("### Total monthly trip duration")

    trip_duration_per_month_chart = alt.Chart(trip_duration_per_month_df).mark_bar().encode(
        x=alt.X("Month:O", title="Month number"),
        y=alt.Y("Trip Duration:Q", title="Trip Duration (days)"),
    )

    st.altair_chart(trip_duration_per_month_chart,
                    use_container_width=True)

    if "Generation" in df.columns:
        trip_duration_per_generation = df[[
            'Trip Duration', 'Generation']].groupby('Generation').sum()
        ...
        st.plotly_chart(trip_duration_per_generation_chart,
                        use_container_width=True)
        ...
        trip_count_per_generation_chart = alt.Chart(trip_count_per_generation_df.rename(
            columns={"index": "Generation",
                     "Generation": "Number of trips"}
        )).mark_bar().encode(
            x=alt.X("Generation:O", sort="y"),
            y=alt.Y("Number of trips:Q"),
        )

        st.altair_chart(trip_count_per_generation_chart,
                        use_container_width=True)
    ...
    months = st.multiselect(
        label="Choose a month (if left unselected, it will get all months)", options=MONTHS, default=MONTHS, format_func=format_label
    )

    days = st.multiselect(
        label="Choose a day (if left unselected, it will get all days)", options=DAYS, default=DAYS, format_func=format_label
    )
    ...
    # Most common month
    st.markdown("* The most common month is: {}".format(
        MONTHS[df['Month'].value_counts().idxmax() - 1].title()))

    # Most common day of week
    st.markdown("* The most common day of week is: {}".format(
        DAYS[df['Day of Week'].value_counts().idxmax() - 1].title()))

    # Most common start hour
    st.markdown("* The most common start hour is: {}".format(
        df['Start Hour'].value_counts().idxmax()))

    # Trip duration stats
    st.markdown("### Trip duration")

    st.markdown("* The total travel time is: {} hours".format(
        round(df['Trip Duration'].sum() / 3600, 2)))

    st.markdown("* The average (mean) travel time is: {} minutes".format(
    ...
    most_frequent_start_stations_df = most_frequent_start_stations.to_frame().reset_index()

    most_frequent_start_stations_chart_data = most_frequent_start_stations_df\
        .rename(
            columns={"index": "Start Station",
                     "Start Station": "Number of Trips"}
        )

    st.markdown("### Most commonly used start stations")

    most_frequent_start_stations_chart = alt.Chart(most_frequent_start_stations_chart_data).mark_bar().encode(
        x='Number of Trips',
        y=alt.Y('Start Station', sort='-x')
    ).properties(
        width="container",
        height=200
    )

    st.altair_chart(most_frequent_start_stations_chart,
                    use_container_width=True)
    ...
    most_frequent_start_stations_chart_data = most_frequent_start_stations_df\
        .rename(
            columns={"index": "End Station",
                     "End Station": "Number of Trips"}
        )
    ...
    st.write(df.groupby(['Start Station', 'End Station']).size().sort_values(
        ascending=False).head().to_frame().reset_index().rename(
            columns={0: "Number of Trips"}
    ))

    # User stats (by Gender and Birth Year only if applicable)
    st.write("### Count of users per user type",
    ...
    if "Birth Year" in df.columns:
        ...
        st.markdown("* The most common year of birth is: {}".format(
...
def create_additional_columns(df):
    ...
    if "Birth Year" in df.columns:
        ...
        df['Generation'] = pd.Series(
            data=map(lambda x: x[1], generations), index=map(lambda x: x[0], generations))
